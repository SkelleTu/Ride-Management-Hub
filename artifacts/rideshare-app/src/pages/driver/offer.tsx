...
import { estimateDriverPrice, formatBRL, ROUND_STEP } from "@/lib/pricing";
...
export default function DriverOffer({ params }: { params: { rideId: string } }) {
  ...
  const priceEstimate = distances ? estimateDriverPrice(distances.totalKm) : null;
  const suggestedPrice = priceEstimate?.roundedPrice ?? null;
                ...
                {priceEstimate && (
                  <>
                    <div className="text-xs text-muted-foreground">
                      Tarifa base {formatBRL(priceEstimate.baseFare)} + {formatBRL(priceEstimate.pricePerKm)}/km
                    </div>
                    <div className="flex justify-between text-xs text-muted-foreground">
                      <span>Valor calculado</span>
                      <span className={priceEstimate.rawPrice !== priceEstimate.roundedPrice ? "line-through" : ""}>
                        {formatBRL(priceEstimate.rawPrice)}
                      </span>
                    </div>
                    {priceEstimate.rawPrice !== priceEstimate.roundedPrice && (
                      <div className="flex justify-between text-xs font-semibold text-primary">
                        <span>Arredondado (múltiplos de R$ {ROUND_STEP})</span>
                        <span>{formatBRL(priceEstimate.roundedPrice)}</span>
                      </div>
                    )}
                  </>
                )}
