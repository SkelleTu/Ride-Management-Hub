...
import {
  ...
  Star, Shield, Zap, Route, Smartphone, CreditCard, UserPlus, MessageCircle, Copy, Check,
} from "lucide-react";
...
export default function RoleSelection() {
  ...
  const priceMapUrl = `${window.location.origin}/precos`;

  const handleCopyPriceMap = (e: React.MouseEvent) => {
    e.stopPropagation();
    navigator.clipboard.writeText(priceMapUrl).then(() => {
      setCopied(true);
    ...
    <div className="relative min-h-[100dvh] flex flex-col items-center justify-center p-4 bg-background overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="grid gap-4">
          ...
          {/* Price map link card */}
          <Card
            className="group cursor-pointer hover:border-primary/40 transition-colors bg-card hover-elevate overflow-hidden relative"
            onClick={() => window.open(priceMapUrl, "_blank")}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-primary/8 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-foreground leading-tight">Mapa de Preços · Araras-SP</p>
                <p className="text-xs text-muted-foreground truncate mt-0.5">{priceMapUrl}</p>
              </div>
              <button
                onClick={handleCopyPriceMap}
                className="shrink-0 w-8 h-8 rounded-lg bg-secondary hover:bg-primary/20 flex items-center justify-center transition-colors"
                title="Copiar link"
              >
                {copied
