...
import { MapPin, Info, Star, ArrowRight, Calculator } from "lucide-react";
...
function updateOverlay(map: any, L: any, svg: SVGSVGElement) {
  ...
  svg.innerHTML = `
    ...
    <circle cx="${center.x.toFixed(1)}" cy="${center.y.toFixed(1)}" r="${radius.toFixed(1)}" fill="url(#pg)"/>
  `;
}
...
export default function PricesPublic() {
  ...
  return (
    <div className="relative h-[100dvh] flex flex-col bg-[hsl(228_22%_8%)] overflow-hidden font-sans">

      {/* Header */}
      <div className="shrink-0 px-4 pt-10 pb-3 bg-black/60 backdrop-blur z-20 border-b border-white/10">
        <div className="flex items-center gap-2.5">
          <MapPin size={18} className="text-primary" />
          <div>
            <p className="text-sm font-bold text-white leading-tight">Mapa de Preços · Araras-SP</p>
            <p className="text-xs text-white/50">A partir da Praça Barão de Araras</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="px-4 pt-4 pb-1">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <p className="text-sm font-bold text-white">Calcule o preço da sua corrida</p>
          </div>
          <PriceEstimator />
        </div>
        ...
        <div className="relative h-72 mt-3">
          ...
          {/* Legend overlay */}
          <div className="absolute bottom-3 left-3 right-3 z-[600] bg-black/80 backdrop-blur rounded-2xl border border-white/10 px-4 py-3">
            <div className="flex items-center gap-1.5 mb-2">
              <Info size={11} className="text-white/40" />
              <span className="text-[11px] text-white/40">Preço máximo · partida da Praça Barão</span>
            </div>
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <div key={item.label} className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full shrink-0" style={{ background: item.color, boxShadow: `0 0 4px ${item.color}` }} />
                  <span className="text-[10px] text-white/50">
                    <strong className="text-white">{item.price}</strong> {item.label}
                  </span>
                </div>
              ))}
            </div>
      ...
      {/* CTA */}
      <div className="shrink-0 z-20 bg-black/90 backdrop-blur border-t border-white/10 px-4 pt-4 pb-8">
        <div className="flex items-start gap-2.5 mb-4">
          <div className="w-9 h-9 rounded-xl bg-primary/20 flex items-center justify-center shrink-0 mt-0.5">
            <Star size={16} className="text-primary fill-primary/40" />
          </div>
          <div>
            <p className="text-white font-bold text-sm leading-tight">Seja um Passageiro VIP</p>
            <p className="text-white/50 text-xs mt-0.5 leading-relaxed">
              Solicite corridas com preço justo, sem surpresas. Conheça Araras com a UPcar.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Link href="/auth/login">
            <button className="w-full flex items-center justify-center gap-2 h-10 rounded-xl border border-white/15 text-white/70 text-sm font-medium hover:bg-white/5 transition-colors">
