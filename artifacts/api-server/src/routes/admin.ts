...
import { sendRideDirectedToDriver, sendRideCancelledNotification, sendAsync } from "../lib/whatsapp";
...
adminRouter.patch("/scheduled-rides/:id/reassign", requireAdmin, async (req, res) => {
  ...
  const [updated] = await db.update(ridesTable)
    .set({ driverId: null, directedToDriverId: driverId, schedulingType: "directed", scheduledStatus: "pending_acceptance", status: "open" })
    .where(and(eq(ridesTable.id, id), eq(ridesTable.isScheduled, true)))
    .returning();
  if (!updated) { res.status(404).json({ error: "Scheduled ride not found" }); return; }

  // Notifica o motorista e o passageiro (best-effort)
  ...
  if (driver && passenger && updated.scheduledFor) {
    sendAsync(
