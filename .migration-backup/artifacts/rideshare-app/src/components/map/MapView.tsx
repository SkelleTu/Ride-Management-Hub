...
export default function MapView({ origin, destination, onMapClick, className = "h-full w-full" }: MapViewProps) {
  ...
  useEffect(() => {
    ...
    if (!mapInstance.current) {
      mapInstance.current = L.map(mapRef.current).setView([-23.5505, -46.6333], 13); // Default to São Paulo
  ...
  useEffect(() => {
    ...
    // Clear existing markers if they no longer exist in props
